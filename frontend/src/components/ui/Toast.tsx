  ...
  window.setTimeout(() => {
    setToasts((prev) => {
      ...
      if (toastToRemove?.timerId) {
        window.clearTimeout(toastToRemove.timerId);
      }
  ...
  return (
    <Portal>
      <style>{`
        ...
        .toast-enter { animation: toast-enter 0.25s ease-out forwards; }
      ...
      <div class="fixed z-[9999] bottom-0 right-0 flex flex-col gap-3 p-4 w-full md:max-w-md">
        <For each={toasts()}>
          {(toast) => {
            ...
            return (
              <div
                class={`group pointer-events-auto relative flex w-full items-start space-x-4 overflow-hidden rounded-xl border p-4 pr-8 shadow-lg ${
