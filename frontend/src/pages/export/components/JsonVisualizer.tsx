      ...
      >
        <span class="mr-1 mt-1 text-muted-foreground w-4 flex justify-center flex-shrink-0">
            <Show when={isObject && !isEmpty}>
                 {isExpanded() ? <ChevronDown class="h-3.5 w-3.5" /> : <ChevronRight class="h-3.5 w-3.5" />}
            </Show>
        </span>
        ...
        <Show when={!isObject}>
             <span 
                class={clsx(
                    ...
                    type === "boolean" ? "text-blue-600 dark:text-blue-400" : "text-gray-600 dark:text-gray-400"
                )}
             >
               ...
               <For each={Object.entries(props.data)}>
                   {([key, value], index) => (
                       <JsonNode 
                          ...
                          data={value} 
                          depth={depth + 1} 
                          isLast={index() === Object.keys(props.data).length - 1} 
                       />
                   )}
               </For>
           </Show>
...
export const JsonVisualizer: Component<{ data: any }> = (props) => {
    return (
