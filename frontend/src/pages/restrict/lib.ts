...
import { restrict, isPathClear } from "~/lib/api";
...
export const executeRestrict = async (
  restrictQuery: setOperationInput,
  spacePath: string
) => {
  if (restrictQuery.pattern.length < 1) {
    showToast({
      title: "Error",
  ...
  if (restrictQuery.template.length > 1) {
    showToast({
      title: "Error",
  ...
  try {
    ...
    const success = await restrict(restrictQuery);
