...
import {
  Item,
  RestrictInput as RestrictInputComponent,
} from "./components/RestrictInput";
...
const RestrictPage: Component = () => {
  ...
  const buildRestrictSExpr = (patterns: Item[]) => {
  ...
  const buildRestrictSetInput = (state: Store<AppState>) => {
  ...
  const handleRestict = () => {
    const restrictQueryInput: setOperationInput = buildRestrictSetInput(state);
    executeRestrict(restrictQueryInput, formatedNamespace());
  };
  ...
  const canRestrict = () => {
    return state.templates.length === 1 && state.patterns.length === 2;
  };

  const copyExpression = () => {
    navigator.clipboard.writeText(buildRestrictSExpr(state.patterns));
  ...
  return (
    <div class="ml-10 mt-8">
      ...
      >
              ...
              <Card class="sticky top-4">
                ...
                <CardContent>
                  <pre class="text-sm font-mono bg-muted p-3 rounded overflow-auto">
                    {buildRestrictSExpr(state.patterns)}
                  </pre>
                  <Button
                    variant="default"
                    size="sm"
        ...
        <Button
          onClick={handleRestict}
          disabled={isLoading() || isPolling() || !canRestrict()}
        ...
        >
          ...
          <Show
            when={isLoading()}
            fallback={
              <Show when={isPolling()} fallback={"Run Restrict"}>
                Waiting for results...
              </Show>
            }
          >
