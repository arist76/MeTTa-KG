...
const parserRaw = LRParser.deserialize({
  version: 14,
  states:
  ...
  specialized: [
    {
      term: 14,
      get: (value: string) =>
        (spec_IDENTIFIER as Record<string, number>)[value] || -1,
    },
  ],
  tokenPrec: 209,
});

export const parser = parserRaw.configure({
  props: [
    indentNodeProp.add({
      Expression: (context) => context.column(context.node.from) + 1,
    }),
    foldNodeProp.add({
    ...
    //   SpaceReference: t.namespace,
    //   GroundedFunction: t.keyword,
...
export const METTA_KEYWORDS = Object.keys(spec_IDENTIFIER);
