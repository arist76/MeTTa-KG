...
fn index(ui: &State<UiConfig>) -> Option<(ContentType, Vec<u8>)> {
    ...
    // ensure backend_url is properly formatted and inject it
    let backend_url = if ui.backend_url.ends_with('/') {
        ui.backend_url.trim_end_matches('/').to_string()
    } else {
        ui.backend_url.clone()
    };
    
    println!("Injecting backend URL: {}", backend_url); // Debug logging
    
    // inject backend URL as a global JS variable
    ...
    if let Some(head_pos) = html.find("</head>") {
        html.insert_str(head_pos, &backend_script);
    } else {
        // fallback: try to find <head> and insert before it closes
        if let Some(head_start) = html.find("<head>") {
            if let Some(head_end) = html[head_start..].find('>') {
                let insert_pos = head_start + head_end + 1;
                html.insert_str(insert_pos, &backend_script);
            }
        } else {
            // last resort: insert at the beginning
            html.insert_str(0, &backend_script);
        }
    }
...
async fn build_rocket(cfg: &Cli) -> Rocket<Build> {
    ...
    // compute backend URL and create UiConfig
    let backend_url = format!("http://{}:{}", cfg.address, cfg.port);
    let ui_config = UiConfig {
        base_path: cfg.base_path.clone(),
        backend_url,
    };

    println!("Server starting with backend_url: {}", ui_config.backend_url); // Debug logging

    // handle base path for mounting - ensure it starts with /
    let mount_path = if cfg.base_path == "/" {
        "/"
    } else {
    ...
    rocket::custom(figment)
        ...
        .mount(mount_path, routes![index, asset, spa_fallback])
}
