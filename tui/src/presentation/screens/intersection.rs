...
impl Screen for IntersectionScreen {
    ...
    fn update(&mut self) {
        ...
        if let Some(result) = result {
            match result {
                Ok(true) => self.status = OperationStatus::Completed("Intersection completed".to_string()),
                Ok(false) => self.status = OperationStatus::Failed("Intersection returned false".to_string()),
            ...
            KeyCode::Enter => {
                ...
                if let Some(service) = service {
                    ...
                    tokio::spawn(async move {
                        let result = service.intersection(&input).await.map_err(|e| e.to_string());
