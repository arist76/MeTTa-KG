...
impl Screen for CompositionScreen {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> Option<ScreenAction> {
        match key.code {
            KeyCode::Tab => {
                self.focused.0 = (self.focused.0 + 1) % self.field_count;
                None
            }
            ...
            KeyCode::Enter => {
                ...
                if let Some(service) = service {
                    let input = SetOperationInput {
                        ...
                        target: vec![self.target.clone()],
                    };
                    tokio::spawn(async move { let _ = service.composition(&input).await; });
                }
